...
@tbl-cleaned-dataset-preview presents the first 5 rows from the cleaned dataset, which contains information on children's book.
...
@fig-page-count shows that `high-rated` books have more page count than low-rated books. Highly-rated books had an average page count of 66.1, compared to the 57.6 average page count of low-rated books.
...
This study employed a Bayesian logistic regression model to analyze the relationship between children book's `high-rated` status and the characteristic of the book. The model is as follows:
...
@fig-interval represent the 90% probability intervals of the estimates, where a coefficient is considered statistically significant if the range excludes 0. In addition to this, these coefficients are expressed in log-odds, meaning it suggests a positive association with a book being high-rated when positive and low-rated when negative. 

With this in mind, we can see that, the Kindle Edition cover type had the highest correlation with a book being `high-rated`, assuming all else held constant, reflecting the analysis that was seen in @sec-data. In contrast, the number of pages of a book, and the number of ratings no statistically significant influence on a the likelihood of a book being high-rated.

Although the publish year was not statistically significant according to the @fig-interval, it exhibited a slight negative relationship (coefficient: -0.01), consistent with previous findings @fig-publish-year that `high-rated` books tend to have earlier publication years compared to low-rated ones.
...
In @fig-post-prior we compared the posterior with the prior, to examine how the estimates change once data is taken into account (CITE ALEXANDER). Most of the variables do not vary even after data was taken into account, showing that the observed data matches the expectations of a "highly-rated" book.
...
@fig-trace and @fig-rhat are the trace plot of the model and Rhat plot of the model respectively. The trace plot shows oscillating horizontal lines, with overlaps between chains, showing no signs of issues with the model. Similarly, the Rhat plot doesn't indicate any issues with the model, as all the values remain close to 1 (CITE ALENXANDER)
